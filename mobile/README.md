...
| **Profile** (modal, from the avatar on Home) | Your details, hide balances, Appearance (System / Light / Dark), accounts, Help & support and Privacy policy (open the website's /support and /privacy), Reset demo data (demo mode only), Sign out, and Delete account (Supabase only; permanent, after a confirmation). |
